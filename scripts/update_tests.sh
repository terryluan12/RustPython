...
annotate_lib() {
    ...
    while ! grep -q "Tests result: SUCCESS" <<< "$output"; do
        ...
        readarray -t failed_tests <<< "$(echo "$output" | awk '/^(FAIL:|ERROR:|UNEXPECTED SUCCESS:)/ {print $(NF-1)}' | sort -u)"

        # If the test fails/errors, then expectedFailure it
        for test in "${failed_tests[@]}"
        do
            error=$(echo "$output" | pcregrep -M -o2 "(?s)======================================================================\n\s*(FAIL:|ERROR:) $test.*?\n----------------------------------------------------------------------.*\n(.*)\n\n(======================================================================|----------------------------------------------------------------------)" 2>/dev/null)
            if echo "$output" | grep -q "UNEXPECTED SUCCESS: $test"; then
                replace_expected_with_skip $rlib_path $test "Unexpected success $error"
            elif already_failed $rlib_path $test; then
                replace_expected_with_skip $rlib_path $test "$error"
            else
                add_above_test $rlib_path $test "$RUSTPYTHON_CANONICAL_EX_FAILURE; $error"
            fi
        done
...
rustpython() {
    target/release/rustpython -m test -j 1 -u all --fail-env-changed --timeout "$timeout" -v "$@"
}
...
replace_expected_with_skip() {
    ...
    test_name=$2
    error=$3
    sed -E "/$RUSTPYTHON_CANONICAL_EX_FAILURE_RE/ { N; /\n\s*def $test_name/ { s/^(\s*)@unittest\.expectedFailure\s+# TODO: RUSTPYTHON.*\n/\1@unittest.skip\('TODO: RUSTPYTHON; $error'\)\n/ } }" -i $file
}
